...
To pull company contribution data, the application will first need to pull the relevant payroll run for the customer's organization. To retrieve the payroll run information, you application can retrieve the Organization's payroll runs using the Payroll Runs endpoint.
...
Then using the appropriate payrollRunId, retrieve company contribution and employee contribution amounts using the Employee Payroll Run.
...
```js
curl --location --request GET 'https://api.rippling.com/platform/api/payroll_runs/5e94fb906bbe6b32863ce57e/employees' \
--header 'Authorization: Bearer WuI39OrWsEHn16dqcIFYKqkHG2qK9H'
```
...
What is the recommended process for pulling employee contribution data (let's say someone was building against our APIs and just wanted to build the 180 part). Based on our endpoints, I would say call GET /payroll_runs -> (retrieve payrollRunId) -> GET /payroll_runs/{payrollRunId}/employees -> (retrieve companyContributionAmt & employeeDeductionAmt for deductions that a relevant 401(k) deductionTypeEnum's) -> Pull a filtered list of employees based on the plan and employee attributes from GET /Employees -> Match the relevant employees with the deductions and calculate actual deferral amounts. Then, update 401(k) system for deferral amounts.
10:04
Is that the right process or do you recommend something else?
